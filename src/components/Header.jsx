...
import { Menu, X, User, LogOut, Settings, LayoutDashboard, ChevronDown, ShoppingCart } from "lucide-react";
...
export default function Header() {
  ...
  const navItems = [
    ...
    { label: "Points Shop", path: "/points-shop" },
  ];
    ...
    <header 
      className={`sticky top-0 left-0 right-0 z-50 transition-all duration-500 ${
        isScrolled 
          ...
          : "bg-transparent border-b border-white/[0.07] py-2"
      }`}
    >
      <div 
      ...
      >
        ...
        <div className="hidden lg:flex items-center gap-4">
          <Link
            to="/cart"
            className={`relative px-4 py-2 rounded-full transition-all duration-300 border flex items-center gap-1.5 font-mono text-[9px] uppercase tracking-widest ${
        ...
        >
          <nav className="flex flex-col gap-5 text-sm font-mono tracking-widest uppercase text-[#9A9DA6]">
            ...
            <Link 
              ...
              onClick={() => setIsOpen(false)}
              className={`flex items-center justify-between py-2.5 border-b border-white/[0.04] transition-colors ${
                location.pathname === "/cart" ? "text-[#A78BFA] font-bold" : "hover:text-[#EDECE7]"
              }`}
            >
              ...
              {location.pathname === "/cart" && <div className="w-1.5 h-1.5 rounded-full bg-[#A78BFA] shadow-[0_0_6px_#A78BFA]"></div>}
            </Link>
          </nav>
            ...
            {user ? (
              <div className="space-y-4">
                ...
                <button 
                  ...
                  className="w-full bg-[#7C5CFC] hover:bg-[#6D4DEF] text-white text-xs font-bold uppercase tracking-widest py-3.5 rounded-xl transition-all cursor-pointer shadow-lg shadow-[#7C5CFC]/20"
                >
                  Go to Dashboard
                </button>
